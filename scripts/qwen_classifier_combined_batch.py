"""
Batched version of qwen_classifier_combined.py for big GPUs (A100 etc.), using vLLM.
Same usage, prompt and output JSON, but many image pairs are in flight at once so the
...
Layout:  <data_dir>/<episode>/<modality>/<frame>.png, with an RGB and a thermal folder
         in each episode (see qwen_classifier_combined.py)
...
Usage:
    python qwen_classifier_combined_batch.py ../data
    python qwen_classifier_combined_batch.py ../data --model Qwen/Qwen3.5-4B --bits 16 --samples 5
...
# Same image loading and engine settings as the one-image batch script,
# same pairing and prompt as the one-at-a-time combined script
from qwen_batch_classifier import CHUNK, MAX_MODEL_LEN, QUANT, load
from qwen_classifier import KPT_NAMES, PROMPT, consensus, frame_number, parse
from qwen_classifier_combined import HINT, find_pairs
...
def main():
    ap = argparse.ArgumentParser(description="Qwen human detection on RGB + thermal pairs (batched, vLLM) -> viewer JSON")
    ...
    # One prompt for every pair: color image, thermal image, instructions.
    ...
    with ThreadPoolExecutor(8) as pool:  # decode PNGs in parallel while nothing else runs
        ...
        for ep in sorted(p for p in args.data_dir.iterdir() if p.is_dir()):
            ...
            for i in range(0, len(pairs), CHUNK):
                ...
                reqs = [{"prompt": text, "multi_modal_data": {"image": [img for img, _ in pair]}} for pair in imgs]
                ...
                for paths, pair, reply, run in zip(chunk, imgs, replies, runs):
